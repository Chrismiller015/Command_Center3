...
  registerOsHandlers();
  ...
  // *** ADDED: Handler for providing the <webview> preload script path ***
  // This is requested by the PluginView component to securely set up the plugin's environment.
  ipcMain.handle('get-preload-path', () => {
    // __dirname resolves to the `out/main` directory. The preload script is in `out/preload`.
    // We construct the absolute path to the bundled preload script.
